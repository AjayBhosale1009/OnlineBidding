...
# allows new user to create account
...
# allows users to create a new auction lisitng
@login_required(login_url='login')
def create(request):
    if request.method == "POST":
        m = auctionlist()
...
# displays details of auction item
...
@login_required(login_url='login')
def bid(request):
    # Checks if a user's bid is greater than the current minimum required bid.
...
def win_ner(request):
    # Determines the winner of an auction after it is closed.
    ...
    # Updates the auction's active_bool status to mark it as closed.
...
# Displays a list of all auctions the logged-in user has won.
...
# this language model capable of performing instruction-based text generation tasks, including answering questions or engaging in conversational interactions.
API_URL = "https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.2"
headers = {"Authorization": "Bearer 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"}
